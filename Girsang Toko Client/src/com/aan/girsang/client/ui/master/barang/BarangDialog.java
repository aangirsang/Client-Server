 ...
 */
package com.aan.girsang.client.ui.master.barang;
...
import com.aan.girsang.api.util.Perhitungan;
...
import com.aan.girsang.client.ui.master.golongan.PilihGolonganBarang;
...
public class BarangDialog extends javax.swing.JDialog {
    ...
    public Barang showDialog(Barang p, String Title) {
        ...
        Clear();
        if (p == null) {
            txtPLU.setText(ClientLauncher.getConstantService().ambilBerikutnya(MasterRunningNumberEnum.BARANG));
        } else {
            loadModelToForm(p);
            isiTabelHistory(p);
            isiTabelHPP(p);
    ...
    private void isiTabelHistory(Barang barang) {
        listHistory = ClientLauncher.getTransaksiService().cariBarang(barang);
        tblHistoryPembelian.setModel(new HistoryTabelModel(listHistory));
        ukuranTabelHistory();
    }
    private void isiTabelHPP(Barang barang) {
        listHPP = ClientLauncher.getMasterService().hPPBarangs(barang);
...
//</editor-fold>
        ...
        btnBaruHPP.addActionListener((ActionEvent ae) -> {
            if("Baru".equals(btnBaruHPP.getText())){
                ...
                ClientLauncher.getMasterService().simpan(hPPBarang);
                isiTabelHPP(barang);
...
//</editor-fold>
    ...
    private void btnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCariActionPerformed
        ...
        GolonganBarang g = (GolonganBarang) new PilihGolonganBarang().showDialog(judul);
        isiComboGolongan();
        if (g != null) {
